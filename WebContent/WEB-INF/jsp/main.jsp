...
<s:iterator value="products" var="p" status="a">
	...
	<div class="dd2">$${p.price}</div>
	  
		<div class="dd3">${p.name}</div>
		<div class="dd4">数量为${p.psize} 件</div>
		<div onclick="" class="dd5"><a href="getProduct?product.id=${p.id}" class="a1" >立即查看</a></div>
	</div>
	<!--<s:if test="#a.index>0">
...
<div>
<div class="footer1" style="text-align:center; width:500px; margin:0px auto;">
        <a href="listProduct?page.start=0">首  页</a>
    
        <a href="listProduct?page.start=${page.start-page.count}">上一页</a>
        <a href="listProduct?page.start=${page.start+page.count}">下一页</a>
        <a href="listProduct?page.start=${page.last}">末  页</a>
    </div>
